...
public class CheeseSoundTrigger : MonoBehaviour
{

    AudioSource source; // AudioSource component to play sound
    Collider soundTrigger; // Collider to detect when the player enters the trigger area
    ...
    void Awake()
    {
        ...
        soundTrigger = GetComponent<Collider>(); // Get the Collider component attached to this GameObject
    }
   
   private void OnTriggerEnter(Collider other) {
        if (other.CompareTag("Mouse_Player")) // Check if the object entering the trigger is tagged as "Player" and sound hasn't been played yet
        {
